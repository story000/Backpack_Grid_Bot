...
class BpxClient:
    ...
    def ExeOrders(self, orders: list):
        """
        ...
        """
        return self._handle_bpx_request(
            url=f'{self.url}api/v1/orders',
            headers=self.sign('orderExecute', orders),
            params=orders,  
            r_type='POST'
        )
    ...
    def build_sign(self, instruction: str, ts: int, params=None):
        if not isinstance(params, list):
            ...
            for order in params:
                ...
                if order is None:
                    order = {}
                if 'postOnly' in order:
                    order = order.copy()
                    order['postOnly'] = str(order['postOnly']).lower()
                sorted_params = "&".join(
                    f"{key}={value}" for key, value in sorted(order.items())
                )
                if sorted_params:
                    sign_str += "&" + sorted_params
                if self.debug and self.debugTs > 0:
